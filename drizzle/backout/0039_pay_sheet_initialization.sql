...
BEGIN
	IF EXISTS (
		SELECT 1
		FROM "audit_events"
		WHERE "action"::text = 'pay_sheet_initialized'
	) THEN
		RAISE EXCEPTION 'pay-sheet initialization audit action is in use; preserve audit history and forward-fix'
			USING ERRCODE = '55000',
				CONSTRAINT = 'pay_sheet_initialization_audit_action_in_use';
	END IF;
END;
...
ALTER FUNCTION "sync_mga_payment_sheet_placement_without_lazy_init"(
	...
	boolean,
	timestamp with time zone
) RENAME TO "sync_mga_payment_sheet_placement";
--> statement-breakpoint
DROP FUNCTION "initialize_pay_sheet_owner_chain"(
	uuid,
...
ALTER TYPE "audit_action" RENAME TO "audit_action_with_pay_sheet_initialization";--> statement-breakpoint
...
DROP TYPE "audit_action_with_pay_sheet_initialization";--> statement-breakpoint
