DO $$
BEGIN
	IF EXISTS (SELECT 1 FROM "policy_change_requests")
		OR EXISTS (
			SELECT 1
			FROM "audit_events"
			WHERE "entity_type" = 'policy_change_request'
				OR "action" IN (
		...
		RAISE EXCEPTION 'policy change-request history is in use; preserve it and forward-fix'
			USING ERRCODE = '55000',
				CONSTRAINT = 'policy_change_request_history_in_use';
	END IF;
END;
$$;--> statement-breakpoint
DROP FUNCTION "resolve_corrected_policy_change_request"(
	uuid,
	uuid,
	text,
	uuid,
...
DROP FUNCTION "send_back_policy_change_request"(
	uuid,
	uuid,
	text,
	timestamp with time zone
);--> statement-breakpoint
DROP FUNCTION "resolve_policy_change_request_as_is"(
	uuid,
	uuid,
	timestamp with time zone
);--> statement-breakpoint
DROP FUNCTION "create_policy_change_request"(
	uuid,
	uuid,
	text,
	timestamp with time zone
);--> statement-breakpoint
DROP TABLE "policy_change_requests";--> statement-breakpoint
DROP FUNCTION "enforce_policy_change_request_write_path"();--> statement-breakpoint
DROP TYPE "policy_change_request_resolution";--> statement-breakpoint
DROP TYPE "policy_change_request_status";--> statement-breakpoint
...
ALTER TYPE "audit_action" RENAME TO "audit_action_with_policy_change_requests";--> statement-breakpoint
...
DROP TYPE "audit_action_with_policy_change_requests";--> statement-breakpoint
ALTER TYPE "audit_entity_type" RENAME TO "audit_entity_type_with_policy_change_requests";--> statement-breakpoint
...
DROP TYPE "audit_entity_type_with_policy_change_requests";--> statement-breakpoint
