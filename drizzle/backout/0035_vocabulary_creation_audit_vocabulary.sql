...
BEGIN
	IF EXISTS (
		...
		OR "entity_type"::text IN (
			'carrier',
			'policy_type',
			'mga'
		)
	) THEN
		RAISE EXCEPTION 'vocabulary creation audit values are in use; preserve audit history and forward-fix'
			USING ERRCODE = '55000',
				CONSTRAINT = 'vocabulary_creation_audit_values_in_use';
	END IF;
END;
...
ALTER TYPE "audit_action" RENAME TO "audit_action_with_vocabulary_creation";--> statement-breakpoint
ALTER TYPE "audit_entity_type" RENAME TO "audit_entity_type_with_vocabulary_creation";--> statement-breakpoint
...
DROP TYPE "audit_action_with_vocabulary_creation";--> statement-breakpoint
DROP TYPE "audit_entity_type_with_vocabulary_creation";--> statement-breakpoint
