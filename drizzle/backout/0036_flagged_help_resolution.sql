...
BEGIN
	IF EXISTS (
		SELECT 1
		FROM "audit_events"
		WHERE "action" = 'draft_help_withdrawn'
			OR (
				"action" = 'draft_sent_back'
				AND "entity_type" = 'draft'
			)
	) THEN
		RAISE EXCEPTION 'flagged-help resolution audit history is in use; preserve it and forward-fix'
			USING ERRCODE = '55000',
				CONSTRAINT = 'flagged_help_resolution_history_in_use';
	END IF;
END;
$$;--> statement-breakpoint
DROP FUNCTION "withdraw_flagged_help"(
	uuid,
	uuid,
	timestamp with time zone
);--> statement-breakpoint
DROP FUNCTION "send_back_flagged_draft"(
	uuid,
	uuid,
	text,
	timestamp with time zone
...
ALTER TYPE "audit_action" RENAME TO "audit_action_with_flagged_help_resolution";--> statement-breakpoint
...
DROP TYPE "audit_action_with_flagged_help_resolution";--> statement-breakpoint
