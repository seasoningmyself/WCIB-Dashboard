...
BEGIN
	IF EXISTS (
		...
		WHERE "capability" = 'support_engineer'
	) OR EXISTS (
		SELECT 1
		FROM "users"
		WHERE "last_login_at" IS NOT NULL
	) THEN
		RAISE EXCEPTION 'support-engineer history is in use; preserve it and forward-fix'
			USING ERRCODE = '55000',
				CONSTRAINT = 'support_engineer_history_in_use';
	END IF;
END;
...
ALTER TYPE "audit_action" RENAME TO "audit_action_with_support_engineer";--> statement-breakpoint
...
DROP TYPE "audit_action_with_support_engineer";--> statement-breakpoint
ALTER TYPE "audit_entity_type" RENAME TO "audit_entity_type_with_support_engineer";--> statement-breakpoint
...
DROP TYPE "audit_entity_type_with_support_engineer";--> statement-breakpoint
